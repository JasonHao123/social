...
final class UserCookieGenerator {
	...
	private final CookieGenerator socialCookieGenerator = new CookieGenerator();
	public UserCookieGenerator() {
		userCookieGenerator.setCookieName("quickstart_user");
		socialCookieGenerator.setCookieName("social_user");
	}

	public void addCookie(String userId, String userKey, HttpServletResponse response) {
		userCookieGenerator.addCookie(response, userId);
		socialCookieGenerator.addCookie(response, userKey);
	...
	public String readCookieValue2(HttpServletRequest request) {
        Cookie[] cookies = request.getCookies();
        if (cookies == null) {
            return null;
        }
        for (Cookie cookie : cookies) {
            if (cookie.getName().equals(socialCookieGenerator.getCookieName())) {
                return cookie.getValue();
            }
        }
        return null;
